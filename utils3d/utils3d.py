...
class Utils3D:
    ...
    @staticmethod
    def write_landmarks_as_ply_external(file_name):
        # input : file landmarks.txt
        path = 'assets/obj_name.txt'                                                    # default path
        
        cloud = PyntCloud.from_file(file_name ,sep=" ", header=0, names=["x","y","z"] )
        name_lm_ply = os.path.splitext(file_name)[0] + '.ply'                           # this is filename.ply
        cloud.to_file(name_lm_ply)                                                      # save landmarks point as ply file
        
        obj_filename = open(path, 'a')
        obj_ply_name = name_lm_ply + '\n'
        obj_filename.write(obj_ply_name)
        obj_filename.close()
